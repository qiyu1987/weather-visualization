...
class Visualization extends SizedComponent{
    ...
    getBars(){
        ...
        for(const d of this.props.model.data){
            ...
            const y = this.getY(d.max_temp);
            bars.push(<rect x={x} y={y} width={w} height={bottom-y} key={d.date} style={barStyle} />);
            x += barWidth;
        }
    ...
    getGrid(){
        ...
        for(const temp of temperatures){
            ...
            grid.push(<g key={temp}>
                <line   x1={this.props.margins.left}
                        x2={x2}
                        y1={y}
                        y2={y}
                        style={styleGridLine} />
                <text x={x2 + 10} y={y+4} style={styleGridText}>{temp}</text>
            </g>);
        }
    ...
    render(){
        ...
        if(this.props.model.status === 'success'){
            ...
            output = <svg width={this.state.width} height={this.props.height}>
                <g>
                    {bars.map((item) => (item))}
                </g>            
                <g>
